...
async function startServer() {
  ...
  app.get('/api/patrols', (req, res) => {
    ...
    if (ranger_id) sql += ` AND ranger_id = '${ranger_id}'`;
  ...
  app.put('/api/patrols/:id', (req, res) => {
    ...
    try { run("UPDATE patrols SET end_time=?,end_lat=?,end_lng=?,status=?,notes=?,updated_at=datetime('now') WHERE id=?", [end_time,end_lat||null,end_lng||null,status,notes||'',req.params.id]); res.json({success:true}); }
    catch (e) { res.status(400).json({ error: e.message }); }
  });
  ...
  app.get('/api/observations', (req, res) => {
    ...
    if (patrol_id) sql += ` AND patrol_id = '${patrol_id}'`;
  ...
  app.post('/api/sync', (req, res) => {
    ...
    try {
      ...
      for (const c of ck) { run("INSERT OR IGNORE INTO checkins (id,ranger_id,patrol_id,lat,lng,status,recorded_at,synced_at) VALUES (?,?,?,?,?,?,?,datetime('now'))", [c.id,c.ranger_id,c.patrol_id||null,c.lat||null,c.lng||null,c.status,c.recorded_at]); r.checkins++; }
